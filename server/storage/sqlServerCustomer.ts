...
/** Trim trailing whitespace from SQL Server CHAR/NCHAR columns */
function trimStr(value: any): string | null {
  return typeof value === 'string' ? value.trimEnd() : value ?? null;
}
...
function mapCustomerFromDb(row: any): Customer {
  return {
    ...
    preferredName: trimStr(row.preferred_name),
    title: trimStr(row.title),
    suffix: trimStr(row.suffix),
    dateOfBirth: row.date_of_birth,
    gender: trimStr(row.gender),
    maritalStatus: trimStr(row.marital_status),
    businessName: trimStr(row.business_name),
    fullName: trimStr(row.full_name),
    taxIdentifier: trimStr(row.tax_identifier),
    governmentId: trimStr(row.government_id),
    governmentIdType: trimStr(row.government_id_type),
    citizenship: trimStr(row.citizenship),
    customerType: trimStr(row.customer_type),
    customerStatus: trimStr(row.customer_status),
    customerSince: row.customer_since,
    kycStatus: trimStr(row.kyc_status),
    kycLastUpdated: row.kyc_last_updated,
    riskRating: trimStr(row.risk_rating),
    languagePreference: trimStr(row.language_preference),
    occupation: trimStr(row.occupation),
    employerName: trimStr(row.employer_name),
    naicsCode: trimStr(row.naics_code),
    dbaName: trimStr(row.dba_name),
    branchId: row.branch_id,
    jackHenryCifNumber: trimStr(row.jack_henry_cif_number),
    silverlakeCustomerId: trimStr(row.silverlake_customer_id),
    inquiryCode: trimStr(row.inquiry_code),
    insideCode: trimStr(row.inside_code),
    salesAssociateCode: trimStr(row.sales_associate_code),
    classCode: trimStr(row.class_code),
